	...
	//determine which mode is being requested
		switch ($key) {
			...
			case 'create':
				...
				try{
					$sql = "INSERT INTO technology(name, description) VALUES(:name, :description)";
			...
			//update an existing record in the technology table
			case "update":
				...
				try {
					$sql = "UPDATE technology SET name =:name, description =:description WHERE id =:recordID";
					$stmt = $pdo->prepare($sql);
					$stmt->bindParam(':name', $name, PDO::PARAM_STR);
					$stmt->bindParam(':description', $description, PDO::PARAM_STR);
			...
			//remove an existing record in the technology table
				case "delete";
				...
				try{
					...
					$sql = "DELETE FROM technology WHERE id = :recordID"
					$stmt = $pdo->prepare($sql);
            		$stmt->bindParam(':recordID', $recordID, PDO::PARAM_INT);
            		$stmt->execute();
