...
header('Access-Control-Allow-Origin');
	...
	//determine which mode is being requested
		switch ($key) {
			...
			//add new record to the technologies table
			case 'create':
				...
				try{
					$sql = "INSERT INTO privateroom(room_name, passenger_name, time_start, time_end, remarks) VALUES(:room_name, :passenger_name, :time_start, :time_end, :remark)";
			...
			case "update":
				...
				try {
					$sql = "UPDATE privateroom SET room_name =:room_name, passenger_name =:passenger_name, time_start =:time_start, time_end =:time_end, remarks =:remarks WHERE id =:id";
					$stmt = $pdo->prepare($sql);
					$stmt->bindParam(':room_name', $room_name, PDO::PARAM_STR);
					$stmt->bindParam(':passenger_name', $passenger_name, PDO::PARAM_STR);
					$stmt->bindParam(':time_start', $time_start, PDO::PARAM_STR);
			...
			//remove an existing record in the privateroom table
				case "delete";
				...
				try{
					...
					$sql = "DELETE FROM privateroom WHERE id = :id"
					$stmt = $pdo->prepare($sql);
            		$stmt->bindParam(':id', $id, PDO::PARAM_INT);
            		$stmt->execute();
