...
header('Access-Control-Allow-Origin: *');
	...
	//determine which mode is being requested
		switch ($key) {
			...
			//add new record to the showerroom table
			case 'create':
				...
				try{
					$sql = "INSERT INTO showerroom(room_name, passenger_name, time_start, time_end, remarks) VALUES(:room_name, :passenger_name, :time_start, :time_end, :remark)";
			...
			case "update":
				...
				try {
					$sql = "UPDATE showerroom SET room_name =:room_name, passenger_name =:passenger_name, time_start =:time_start, time_end =:time_end, remarks =:remarks WHERE id =:id";
					$stmt = $pdo->prepare($sql);
					$stmt->bindParam(':room_name', $room_name, PDO::PARAM_STR);
					$stmt->bindParam(':passenger_name', $passenger_name, PDO::PARAM_STR);
					$stmt->bindParam(':time_start', $time_start, PDO::PARAM_STR);
			...
			//remove an existing record in the showerroom table
				case "delete";
				...
				try{
					...
					$sql = "DELETE FROM showerroom WHERE id = :id"
					$stmt = $pdo->prepare($sql);
            		$stmt->bindParam(':id', $id, PDO::PARAM_INT);
            		$stmt->execute();
